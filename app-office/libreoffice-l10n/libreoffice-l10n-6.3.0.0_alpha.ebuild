...
BASE_PV=$(ver_cut 1-3)
MY_PV="${PV/_alpha/.alpha1}"
MY_PV="${MY_PV/_beta/.beta1}"
...
src_prepare() {
	...
	for lang in ${LANGUAGES}; do
		...
		if [[ ${lang%:*} != en ]]; then
			rpmdir="LibreOfficeDev_${MY_PV}_Linux_x86-64_rpm_langpack_${dir}/RPMS/"
			[[ -d ${rpmdir} ]] || die "Missing directory: ${rpmdir}"
			rpm_unpack ./${rpmdir}/*.rpm
		fi
		if [[ "${LANGUAGES_HELP}" =~ " ${lang} " ]] && use offlinehelp; then
			rpmdir="LibreOfficeDev_${MY_PV}_Linux_x86-64_rpm_helppack_${dir}/RPMS/"
			[[ -d ${rpmdir} ]] || die "Missing directory: ${rpmdir}"
			rpm_unpack ./${rpmdir}/*.rpm
		fi
	done
...
src_install() {
	local dir="${S}"/opt/${PN/-l10n/}dev$(ver_cut 1-2)/
