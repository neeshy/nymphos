...
src_prepare() {
	use alsa-oss && sed -i 's/forcealsa=false/forcealsa=true/' "${S}/runescape/share/configs/settings.conf"
	use cario-nogl && rm -rf "${S}/runescape/rsu/3rdParty/linux/cairo-nogl"
	use pulseaudio && sed -i 's/forcepulseaudio=false/forcepulseaudio=true/' "${S}/runescape/share/configs/settings.conf"
	use wine && sed -i 's/compabilitymode=false/compabilitymode=true/' "${S}/runescape/share/configs/settings.conf"

	# Set java path to system-java
	sed -i 's:preferredjava=default-java:preferredjava=/etc/java-config-2/current-system-vm/bin/java:' \
